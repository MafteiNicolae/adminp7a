                            ...
                            @foreach($contors as $linie)
                                <tr>
                                    ...
                                    @for($y=$j; $y<\App\Models\Apartament::max('nrbai')*2+2; $y++)
                                        <td></td>
                                    @endfor
                                    <td><a href="{{route('admin.contors.edit', [$linie->id])}}" class="btn btn-sm btn-primary">Modifica</a></td>
                                    <td>
                                        <form action="{{route('admin.contors.destroy', [$linie->id])}}" method="POST" >@csrf
                                            {{method_field('DELETE')}}
